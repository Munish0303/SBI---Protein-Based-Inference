    ...
    with open(csv_path, newline="") as f:
        for i, row in enumerate(csv.DictReader(f)):
            ...
            if i in want:
                L = int(row["len"])
                obs = encode(row["seq"])
                # Guard against silent CSV<->NPZ misalignment: the two files are paired
                # ONLY by row order, with no checksum. If one is regenerated without the
                # other, seq length and stored-gamma length diverge -> catch it loudly.
                if len(obs) != L or int(lengths[i]) != L:
                    raise ValueError(
                        f"CSV/NPZ misalignment at chain {i}: csv len {len(obs)}, "
                        f"csv 'len' {L}, npz len {int(lengths[i])} -- regenerate both together.")
...
SKIP_START = 2   # positions 0,1: gamma_0 = 0 EXACTLY (start rule) -> logit hits the clip


def build_training_data(train_chains, max_windows, rng, W=WINDOW, skip_start=SKIP_START):
    ...
    `skip_start` drops the first few residues of every chain. Why: the model starts
    in "other" with probability 1, so gamma_0 = 0 EXACTLY for every chain -- the only
    atom in 24.3M residues. logit() clips it to -6.907, which (a) is a point mass a
    continuous flow cannot represent, and (b) inflates the standardisation sd of the
    prev/centre target dims by 13%/7% (next is unaffected -- it never sees the atom).
    Those are exactly the dims that show a positive SBC bias. gamma at these positions
    is known a priori anyway, so there is nothing to learn: drop them.
    """
    per_chain = max(1, max_windows // max(1, len(train_chains)))
    Xs, Ys = [], []
    for obs, gamma, _ in train_chains:
        L = len(obs)
        lo = min(skip_start, max(0, L - 1))          # keep at least one position
        avail = L - lo
        if avail <= 0:
            continue
        k = min(avail, per_chain)
        idx = lo + rng.choice(avail, size=k, replace=False)
        Xw = windows_for_sequence(obs, W)[idx]              # (k, W, N_CH)
        # Boundary approximation: at residue 0, g_prev duplicates gamma_0; at the last
        # residue, g_next duplicates gamma_-1 (there is no real neighbour). Affects ~1-2
        # windows per chain. SKIP_START already drops the front boundary from training;
        # the tail boundary is left in (it is not a deterministic atom, just a repeat).
...
NUM_SAMPLES = 50
"""Canonical number of posterior draws per residue -- used by EVERY evaluation script
so all reported numbers are directly comparable.

Why 50 is enough (argued, then verified):

  * Analytically: the posterior sd is ~0.05 in logit (~0.01 in probability), so the
    Monte-Carlo error on the posterior mean is sd/sqrt(n) ~ 0.01/sqrt(50) ~ 0.0014 --
    5x below the MAE-to-FB (0.007), i.e. already under the noise floor of anything we
    report. Going to 300 only tightens it to 0.0006, which cannot move a printed digit.

  * Empirically (insulin 1MSO, sweeping n over a 10x range):
        chain     n=50     n=150    n=300    n=500
          A     0.5278   0.5185   0.5278   0.5185
          C     0.6019   0.6019   0.6019   0.6019
          B     0.9809   0.9809   0.9809   0.9809
          D     0.9856   0.9809   0.9809   0.9809
    AUC is stable to ~0.001; the B-chain value is identical at every n. The residual
    wobble on the A/C chains is tie-breaking on a 21-residue chain, not MC noise.

Cost matters: the full PISCES sweep is ~2.2M residues, so n=300 means 656 MILLION flow
evaluations (~90 min on CPU) versus ~109 million at n=50 (~15 min). We buy nothing with
the extra 75 minutes, so we don't spend them.
"""
...
def load_model(path=MODEL_PATH):
    """Load the saved approximator. Imported by every eval script (single copy).

    `import bayesflow` first is REQUIRED: it registers the serializable classes, without
    which `keras.saving.load_model` raises a TypeError on the custom flow layers.
    """
...
def predict_helix(approx, obs, num_samples=NUM_SAMPLES, W=WINDOW):
