...
def newest_train_log(here=HERE):
    """Most recently written train_bf*.log.

    Must track the run that produced the CURRENT model -- hard-coding a filename
    silently pairs a new model's SBC with an old run's loss curve.
    """
    import glob
    logs = (glob.glob(os.path.join(here, "logs", "train_bf*.log"))
            + glob.glob(os.path.join(here, "train_bf*.log")))   # logs/ first, root fallback
...
def main():
    ...
    if losses:
        ...
        ax.grid(alpha=0.25)
        fig.tight_layout(); fig.savefig(os.path.join(HERE, "figures", "diag_loss.png"), dpi=130); plt.close(fig)
    ...
    fig.savefig(os.path.join(HERE, "figures", "diag_sbc_ecdf.png"), dpi=130, bbox_inches="tight"); plt.close(fig)
    ...
    fig.savefig(os.path.join(HERE, "figures", "diag_recovery.png"), dpi=130, bbox_inches="tight"); plt.close(fig)
    ...
    fig.savefig(os.path.join(HERE, "figures", "diag_contraction.png"), dpi=130, bbox_inches="tight"); plt.close(fig)
